...
function App() {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${darkMode ? 'bg-gray-900 text-white' : 'bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 text-gray-900'}`}>
      <div className="container mx-auto px-6 py-8 max-w-[900px]">
        <header className="flex justify-between items-center mb-8 bg-white dark:bg-gray-900 p-6 rounded-2xl shadow-lg shadow-black/5">
          <div className="flex items-center space-x-4">
            <div className="w-12 h-12 bg-gradient-to-r from-blue-500 to-purple-500 rounded-2xl flex items-center justify-center text-white font-bold text-xl">WB</div>
            <div>
              <h1 className="text-3xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">AI Website Builder</h1>
              <p className="text-sm text-gray-600 dark:text-gray-400">Create stunning websites with AI</p>
            </div>
          </div>
          <button
      ...
      {/* Feedback Modal */}
      {feedbackModal && (
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50" onClick={() => setFeedbackModal(false)}>
          <div className="bg-white dark:bg-gray-900 p-6 rounded-2xl shadow-lg shadow-black/5 max-w-sm w-full mx-4" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-bold mb-4 text-center text-gray-900 dark:text-white">Feedback</h3>
            {!feedbackSubmitted ? (
              <>
                <div className="flex justify-center space-x-4 mb-4">
                  <button
                    onClick={() => setFeedback({ ...feedback, rating: '👍' })}
                    className={`text-3xl p-2 rounded-lg transition-all ${feedback.rating === '👍' ? 'bg-green-100 dark:bg-green-900' : 'hover:bg-gray-100 dark:hover:bg-gray-800'}`}
                  >
                    👍
                  </button>
                  <button
                    onClick={() => setFeedback({ ...feedback, rating: '👎' })}
                    className={`text-3xl p-2 rounded-lg transition-all ${feedback.rating === '👎' ? 'bg-red-100 dark:bg-red-900' : 'hover:bg-gray-100 dark:hover:bg-gray-800'}`}
                  >
                    👎
                  </button>
                </div>
              ...
              <p className="text-center text-green-600 dark:text-green-400 font-semibold">Thanks for your feedback!</p>
            )}
          </div>
        </div>
      )}
